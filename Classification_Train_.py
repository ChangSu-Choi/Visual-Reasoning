...
#        trainer = pl.Trainer(max_epochs=args.epochs, log_every_n_steps=args.log_every_n_steps, logger=logger, callbacks=[checkpoint_callback], devices=args.devices, strategy="ddp", gpus=1)
        ...
        trainer = pl.Trainer(max_epochs=args.epochs, log_every_n_steps=args.log_every_n_steps, logger=logger, callbacks=[checkpoint_callback], devices=args.devices, strategy="ddp", resume_from_checkpoint=args.resume_from_checkpoint)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_dim", type=int, help="Path to the input file.")
    parser.add_argument("--mlp_hidden", type=int, help="Path to the output file.")
    parser.add_argument("--batch_size", type=int, help="Number of training epochs.")
    parser.add_argument("--model_name", type=str, default=None, help="resnet50 or vit")
    parser.add_argument("--mode", type=str, help="resnet50 or vit")
    parser.add_argument("--num_workers", type=int, help="num_workers for the trainer")
    parser.add_argument("--devices", help="gpus for the trainer")
    parser.add_argument("--epochs", type=int, help="epochs for the trainer")
    parser.add_argument("--clr_temperature",type=float, help="epochs for the trainer")
    parser.add_argument("--learning_rate", type=float, help="learning-rate for the trainer")
    parser.add_argument("--log_every_n_steps", type=int, help="log_every_n_steps for the trainer")
    parser.add_argument("--model_save_path", type=str, help="Path to save the trained model weights.")
    parser.add_argument("--freeze", action='store_true', help="About using freezing model")
    parser.add_argument("--resume_from_checkpoint", type=str, help="Path to the checkpoint to resume training from.")
    parser.add_argument("--train_link", type=str, help="train_link for the trainer")
    parser.add_argument("--valid_link", type=str, help="valid_link for the trainer")
    ...
    if args.task in [1,4]:
        args.train_link = f'/data/Visual-Reasoning/dataset/Classification/task{args.task}/category{args.category}/train'
        args.valid_link = f'/data/Visual-Reasoning/dataset/Classification/task{args.task}/category{args.category}/valid'
    else:
        args.train_link = f'/data/Visual-Reasoning/dataset/Classification/task{args.task}/train'
        args.valid_link = f'/data/Visual-Reasoning/dataset/Classification/task{args.task}/valid'
