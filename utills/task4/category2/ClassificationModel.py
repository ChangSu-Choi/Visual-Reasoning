...
from utills.task1.category4.ClassficationDataset import ClassficationDataset
from utills.task1.category4.get_data import get_data
import torchvision
import timm
import pandas as pd
import torchmetrics
...
class ClassificationModel(pl.LightningModule):
    ...
    def forward(self, x):
        ...
        img_features['q_imgs'] = torch.cat([self.model(img) for img in x['q_imgs']], dim=1)

        a1_img = self.model(x["a1_imgs"])
        a2_img = self.model(x["a2_imgs"])
        a3_img = self.model(x["a3_imgs"])
        a4_img = self.model(x["a4_imgs"])
        a5_img = self.model(x["a5_imgs"])
   
        q_a1 = torch.cat((img_features['q_imgs'], a1_img), dim=1)
        q_a2 = torch.cat((img_features['q_imgs'], a2_img), dim=1)
        q_a3 = torch.cat((img_features['q_imgs'], a3_img), dim=1)
        q_a4 = torch.cat((img_features['q_imgs'], a4_img), dim=1)
        q_a5 = torch.cat((img_features['q_imgs'], a5_img), dim=1)
    ...
    def training_step(self, batch, batch_idx):
        ...
        target_a1, target_a2, target_a3, target_a4, target_a5 = \
